...
export default function AddSocialMedia({ isOpen, onClose }) {
  ...
  const AddNewDepartment = async (data) => {
    ...
    try {
      setisLoading(true);
      const res = await ADD(admin.token, "add_social_media", formData);
      setisLoading(false);
      if (res.response === 200) {
        ShowToast(toast, "success", "Social Media Added!");
