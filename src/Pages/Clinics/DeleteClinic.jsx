...
export default function DeleteClinic({ isOpen, onClose, data }) {
  ...
  const DeleteDeparment = async () => {
    ...
    try {
      setisLoading(true);
      const res = await DELETE(admin.token, "delete_clinic", formData);
      setisLoading(false);
      if (res.response === 200) {
        ShowToast(toast, "success", "Clinic Deleted!");
