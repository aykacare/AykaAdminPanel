...
export default function AddDepartmentModel({ isOpen, onClose }) {
  ...
  const AddNewDepartment = async (data) => {
    ...
    try {
      setisLoading(true);
      const res = await ADD(admin.token, "add_department", formData);
      setisLoading(false);
      if (res.response === 200) {
        ShowToast(toast, "success", "Department Added!");
